...
BEGIN
		SELECT e.eventId, es.ScheduleId,sd.DateId,st.SlotId, e.EventName, e.EventDiscription,
		e.Price, e.EventCapacity, e.Min_Booking, e.Max_Booking, e.EventImage, es.Name,
		sd.Date, cast(STR_TO_DATE(st.time, '%h:%i %p')as datetime) AS times,
		dayname(sd.Date) AS DayName,  DATE_FORMAT(sd.Date,'%M %d %Y') AS FullDate,
		s.Bookings ,s.Available FROM tbl_events e
			LEFT JOIN ( 
...
-- Dump completed on 2022-12-08 18:54:40
